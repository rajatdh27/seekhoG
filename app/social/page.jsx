...
import { friendsAPI, leaderboardAPI, userAPI } from '../utils/api';
...
export default function SocialPage() {
  ...
  const fetchSocialData = async (userId) => {
    setLoading(true);
    try {
      const [friendsRes, requestsRes, sentRes, leaderboardRes] = await Promise.all([
                        ...
                        return (
                          <motion.div layout key={req.id} className="bg-slate-900/60 backdrop-blur-md border border-white/10 p-6 rounded-[2rem] flex flex-col sm:flex-row items-center justify-between gap-6 hover:border-blue-500/40 transition-all shadow-xl">
                              ...
                              <div className="flex items-center gap-3 w-full sm:w-auto">
                                  <button onClick={() => handleAcceptRequest(req.id, formatName(rawName))} disabled={actionLoading === req.id} className="flex-1 sm:flex-none bg-blue-600 hover:bg-blue-500 text-white px-8 py-4 rounded-2xl font-black text-sm flex items-center justify-center gap-2 active:scale-95 transition-all">{actionLoading === req.id ? <Loader2 className="animate-spin w-4 h-4" /> : <Check size={18} />} Accept</button>
                    ...
                    {displayList.length === 0 && !isSearching ? (
                        ...
                        return (
                            <motion.div layout key={person.id || person.username} className="bg-slate-900/40 backdrop-blur-md border border-white/5 p-6 rounded-[2.5rem] hover:border-blue-500/30 transition-all flex flex-col items-center text-center group relative overflow-hidden shadow-lg">
                                <div className="w-24 h-24 rounded-[2rem] bg-slate-800 flex items-center justify-center text-white text-4xl font-black mb-4 border-2 border-white/5 group-hover:scale-105 transition-all shadow-2xl">{formatName(person.username).charAt(0).toUpperCase()}</div>
                                <h3 className="text-xl font-bold text-white mb-1 capitalize tracking-tight">{formatName(person.username)}</h3>
