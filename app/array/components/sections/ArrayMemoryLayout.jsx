...
import { Cpu, Zap, ArrowRight, Layers, Table, Grid3X3, FastForward, TrendingUp, LayoutTemplate, Columns, AlertTriangle, ArrowUpRight } from "lucide-react";
...
export default function ArrayMemoryLayout() {
  ...
  const memoryOrder = [
    { title: "Row-Major Order", description: "Rows stored contiguously. Used by C, C++, Java, JS, Python. Fast row iteration.", icon: LayoutTemplate, color: "blue" },
    { title: "Column-Major Order", description: "Columns stored contiguously. Used by Fortran, MATLAB, R. Fast column iteration.", icon: Columns, color: "cyan" }
  ];
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          <div className="bg-slate-900 border border-white/5 rounded-[2.5rem] p-8 flex flex-col">
            ...
            <div className="flex-1">
              <ConceptGrid items={memoryOrder} columns={1} variant="horizontal" />
            </div>
            <div className="mt-6 flex items-center gap-3 p-4 bg-rose-500/10 border border-rose-500/20 rounded-xl">
              <AlertTriangle size={20} className="text-rose-500 shrink-0" />
              <p className="text-[10px] font-bold text-rose-400 uppercase tracking-tight leading-relaxed">
                Accessing in wrong order can be <span className="text-white">100x slower</span> due to cache misses!
              </p>
            </div>
          </div>
